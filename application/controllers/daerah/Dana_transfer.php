...
class Dana_transfer extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->akses['akses'] == 'Y') {
            ...
            $this->load->view('daerah/dana_transfer/view', $data);
        } else {
    ...
    public function load()
    {
        if ($this->akses['akses'] == 'Y') {
            ...
            $result = $this->mquery->select_by('data_uraian_kegiatan_skpd', ['tahun' => $tahun], "kode_rekening ASC");
            $data = [];
            $no = 0;
            $tahun_now=date('Y');
            foreach ($result as $r) {
                $no++;
                $row_uraian = $this->mquery->select_id('data_kode_rekening', ['kode_rekening' => $r['kode_rekening']]);
                ...
                $data_skpd = $this->mquery->select_id('data_skpd_tahun', ['id_skpd' => $r['id_skpd'], 'tahun' => $tahun]);
    ...
    public function form()
    {
        ...
        if ($opsi == "add") {
            $data = [
                "skpd" => $this->mquery->select_by('data_skpd_tahun', ['tahun'=>$tahun], 'id_skpd ASC'),
                "uraian" => $this->mquery->select_data('data_kode_rekening','id_uraian ASC'),
                "tahun" => $tahun
            ];
            $this->load->view('daerah/dana_transfer/form_add', $data);
        } elseif ($opsi == "edit") {
            ...
            $data = [
                ...
                "skpd" => $this->mquery->select_by('data_skpd_tahun', ['tahun'=>$data_uraian['tahun']], 'id_skpd ASC'),
                "uraian" => $this->mquery->select_data('data_kode_rekening','id_uraian ASC')
            ];
            $this->load->view('daerah/dana_transfer/form_edit', $data);
        } else {
