...
class Tugas extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->akses['akses'] == 'Y') {
            $data = [
                ...
                "submenu_active" => "realisasi-dana-tp"
            ];
            $this->load->view('realisasi/tugas/view', $data);
        } else {
    ...
    public function load()
    {
        if ($this->akses['akses'] == 'Y') {
            ...
            foreach ($result as $r) {
                ...
                $result_realisasi = 0;
                $where = array('id_skpd' => $r['id_skpd'], 'tahun' => $tahun, 'jenis' => 3);
                $result_max = $this->mquery->max_data_where("log_upload_realisasi", "bulan", $where);
                $row_log_upload = $this->mquery->select_id('log_upload_realisasi', ['id_skpd' => $r['id_skpd'], 'tahun' => $tahun, 'jenis' => 3, 'bulan' => $result_max['bulan']]);
                $row_users = $this->mquery->select_id('users', ['id_user' => $row_log_upload['user_input']]);
                $nama_skpd = "<a href=" . base_url("realisasi-dana-tp/detail/" . $tahun . '/' .$encrypt_id) . "><h2>" . $r['nama_skpd'] . "</h2></a>";
                
                $pagu_tugas = $this->mquery->sum_data('tbl_dana_dekon', 'pagu', ['id_skpd' => $r['id_skpd'], 'tahun' => $tahun, 'jenis' => 'TP']);
                $realisasi_tugas = $this->mquery->sum_data('tbl_realisasi_dekon', 'realisasi', ['id_skpd' => $r['id_skpd'], 'tahun' => $tahun, 'bulan' => $result_max['bulan'], 'jenis' => 'TP']);
                
                if($pagu_tugas['pagu']==0){$persen=0;}else
                {$persen=round(($realisasi_tugas['realisasi']/$pagu_tugas['pagu']*100),2);}
                ...
                $row = [
                    ...
                    'dipa' => format_rupiah($pagu_tugas['pagu']),
                    'tampil_tahap' => format_rupiah($realisasi_tugas['realisasi']),
    ...
    public function detail($tahun, $encrypt_id)
    {
        ...
        $this->load->view('realisasi/tugas/view_detail', $data);
    }
    ...
    public function load_apbd()
    {
        ...
        $result = $this->mquery->select_by('log_upload_realisasi', ['id_skpd' => $skpd, 'tahun'=> $tahun, 'jenis'=> 3], "bulan ASC");
        $data = [];
        $no = 0;
        $jenis=2;    //kode Dekon
        foreach ($result as $r) {
            $no++;
            $nama_bulan = "<a href=" . base_url("realisasi-dana-tp/detail2/" .$encrypt_id."/". $tahun."/". $r['bulan']) . ">" . bulan($r['bulan']) . "</a>";
            $row_users = $this->mquery->select_id('users', ['id_user' => $r['user_input']]);
            $pagu_tugas = $this->mquery->sum_data('tbl_dana_dekon', 'pagu', ['id_skpd' => $skpd, 'tahun' => $tahun, 'jenis' => 'TP']);
            $realisasi_tugas = $this->mquery->sum_data('tbl_realisasi_dekon', 'realisasi', ['id_skpd' => $skpd, 'tahun' => $tahun, 'bulan' => $r['bulan'], 'jenis' => 'TP']);
    ...
    {
        $id_skpd = decrypt_url($encrypt_id);
        $row_log_upload = $this->mquery->select_id('log_upload_realisasi', ['id_skpd' => $id_skpd, 'tahun' => $tahun, 'bulan' => $bulan, 'jenis' => 3]);
        $skpd = $this->mquery->select_id('data_skpd', ['id_skpd' => $id_skpd]);

        $result = $this->mquery->select_by(' tbl_dana_dekon', ['id_skpd' => $id_skpd, 'tahun' => $tahun, 'jenis' => 'TP'], "id_dana ASC");
        foreach ($result as $r) {
            $id_dana=$r['id_dana'];
        ...
        $this->load->view('realisasi/tugas/view_detail2', $data);
    }
    ...
    public function load_apbd2()
    {
        ...
        $encrypt_id = encrypt_url($skpd);
        $result = $this->mquery->select_by('tbl_realisasi_dekon', ['id_skpd' => $skpd, 'tahun' => $tahun, 'bulan' => $bulan, 'jenis' => "TP"], "id_dana ASC");
        $data = [];
        $no = 0;
        $tahun_now=date('Y');
        foreach ($result as $r) {
            $no++;
            $id_dana=$r['id_dana'];
            $pagu_tugas = $this->mquery->select_id('tbl_dana_dekon', ['id_dana' => $id_dana]);
            ...
            if($pagu_tugas['pagu']==0){$persen=0;}else
            {$persen=round(($r['realisasi']/$pagu_tugas['pagu']*100),2);}

            $row = [
                'no' => $no,
                'tahun' => $tahun,
                'bulan' => bulan($bulan),
                'kd_satker' =>"Dana : ".$pagu_tugas['jenis']."<br>".$pagu_tugas['kd_satker']."<br>".$pagu_tugas['keterangan'],
                'pagu' => format_rupiah($pagu_tugas['pagu']),
    ...
    public function form_upload()
    {
        ...
        $this->load->view('realisasi/tugas/form_upload', $data);
    }
    ...
    function upload()
    {
        ...
        $this->db->delete('log_upload_realisasi', ['id_skpd' => $id_skpd, 'tahun' => $post['tahun'], 'bulan' => $hsl_bulan, 'jenis' => 3]);
    ...
    public function form()
    {
        ...
        if ($opsi == "add") {
            ...
            $this->load->view('realisasi/tugas/form_add', $data);
        } elseif ($opsi == "edit") {
            ...
            $this->load->view('realisasi/tugas/form_edit', $data);
        } else {
