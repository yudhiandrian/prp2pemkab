<script>
    ...
    function load_apbd() {
        ...
        table = $('#load-content').DataTable({
            ...
            ajax: {
                url: "<?= site_url('realisasi-dana-dak/load_apbd2'); ?>",
                type: 'POST',
                data: {
                    id: "<?= $skpd['id_skpd']; ?>",
                    bulan: "<?=$bulan ?>",
...
$(document).on("click", "#tombol-ubah", function() {
    ...
    $.ajax({
        url: '<?= site_url('realisasi-dana-dak/form'); ?>',
        type: "POST",
        data: {
            opsi: "edit",
            id: $(this).data('id')
...
$(document).on("submit", "#form-ubah", function(e) {
    ...
    $.ajax({
        url: "<?= site_url('realisasi-dana-dak/edit'); ?>",
        type: 'POST',
...
$(document).on("click", "#tombol-hapus", function(e) {
    e.preventDefault();
    url = '<?= site_url('realisasi-dana-dak/delete'); ?>';
    id = $(this).data('id');
