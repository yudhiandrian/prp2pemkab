    ...
    $(document).ready(function() {
        ...
        var chartbar = new Highcharts.Chart({
            chart: {
                ...
                renderTo: 'realisasi-dana-tp-persen',
                type: 'bar'
            },
            title: {
                text: 'Persentase Serapan Belanja Dana Tugas Pembantuan <?=$nama_kabupaten?>'
            },
            subtitle: {
                text: 'Periode 1 januari s.d <?= $this->fungsi->nama_bulan($realisasi_apbd_provinsi['tanggal_data'])?>'
            },
            xAxis: {
                categories: [<?= $realisasi_pb_provinsi['nama_skpd_tp_persen'] ?>],
                labels: {
                    style: {
                        color: '#000000'
                    }
            ...
            series: [{
                ...
                data: [<?= $realisasi_pb_provinsi['persen_tp'] ?>],
                    dataLabels: {
                          enabled: true,
                          formatter: function() {
                            if (this.y < 10) {
